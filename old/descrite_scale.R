#' @export
DiscreteScale<-R6::R6Class("DiscreteScale",
                    public=list(
                      initialize=function(extent=range,domain=NULL,weights=1){
                        self$coord=coord
                        extent<-d_range(extent)
                        self$axis_position=position
                        mark_ids=1:l(  extent)
                        self$domain <-glue('{self$coord}_grp-start {mark_ids}/{self$coord}_grp-end {mark_ids}')
                        domain<-self$domain
                        if(nnull(extent)){
                          self$prv$.scale<- chr_approx(extent,domain)

                          self$prv$.extent<-extent
                        }
