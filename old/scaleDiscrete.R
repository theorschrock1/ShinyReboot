...
ScaleDiscrete <- R6::R6Class(
  "ScaleDiscrete",
  public = list(
  initialize=function(range=NULL,domain=NULL) {
    private$.range = range
    },
  scale_absolute=function(data,domain=NULL){
    range=self$range$numeric_range
    domain=domain%or%self$domain
    approx(range,domain,xout=data)$y
    self$range(data)
  },
  scale_fr=function(data){
    mid=self$range$numeric_range$mid

    approx( mid, mid/median(mid),xout=data)$y
  },
  scale_grid_end=function(data,name='grp'){
    range=self$range$range
    domain=glue("{name}-end {1:l(range)}")
    char_approx(range, domain)(data)

  },
  scale_grid_start=function(data,name='grp'){
    range=self$range$range
    domain=glue("{name}-start {1:l(range)}")
    char_approx(range, domain)(data)
  },
  scale_grid_template=function(data,name){
    data.table(grid_min= scale_grid_start(data,name),grid_mid=.5,grid_max=  scale_grid_end(data,name),grid_bin=scale_fr(data))
  })
  private = list(
    .range=NULL,
    .domain=c(0,100)
...
NRange<-R6::R6Class("ScaleContinuous",
                    public=list(
                      extend_range=0,
