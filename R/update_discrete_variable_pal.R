...
#' @export
update_discrete_variable_pal <- function(inputId, var_names, var_ids = NULL, colors = NULL,palette=NULL, session = getDefaultReactiveDomain()) {
    # update a discrete variable palette input
    ...
    updateSortableDiv(inputId=ms("selected_colors"), content = color_divs)
    updateSortableDiv(inputId=ms("selected_vars"), content = var_divs)
    if (nnull(palette)) {
        shinyWidgets::updatePickerInput(session = session, ms("palette_picker"), selected = palette)
    }
