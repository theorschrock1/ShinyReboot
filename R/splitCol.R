#' Create a column div with resizable rows.
...
#' @export
splitCol=function(...,inputId=NULL,class="",style=NULL,sizes=NULL,minSizes=NULL,expandToMin =TRUE,gutterSize=10,snapOffset=30,dragInterval=1){
#Create a column div with resizable rows
...
if(is.null(sizes))sizes<-rep(1/n,n)
if(is.null(minSizes))minSizes<-rep(30,n)
if(l(minSizes)==1)minSizes<-rep(minSizes,n)
expr_eval(split_div(inputId=inputId,class=paste('d-flex flex-column',class),sizes=sizes,style=style,minSize = minSizes,
          direction="vertical",expandToMin = expandToMin ,gutterSize = gutterSize,snapOffset = snapOffset,dragInterval = dragInterval,!!!more_args, !!!html))
#returns [html]
}
